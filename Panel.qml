...
Panel {
  ...
  readonly property var internalDisplayInfo: findDisplay(internalMonitor)
  readonly property var externalDisplayInfo: findDisplay(externalMonitor)

  // Only offer arrangement for the classic laptop-plus-monitor pair: one
  // internal, one external, both enabled and not mirrored.
  readonly property bool arrangementAvailable: {
    var i = internalDisplayInfo, e = externalDisplayInfo
    return !!i && !!e && i.enabled && e.enabled && !mirrorEnabled
  }

  // Derived from live position, not a remembered choice, so it stays honest
  // if the layout was changed some other way. Left/right and top are told
  // apart by edge overlap, not just the sign of x/y — a left/right pairing
  // also carries a y offset (to line up vertical midpoints), so checking y
  // alone misreads it as "top".
  readonly property string currentArrangement: {
    var i = internalDisplayInfo, e = externalDisplayInfo
    if (!i || !e || i.x === undefined || e.x === undefined) return ""
    var iw = i.width / (i.scale || 1)
    var ih = i.height / (i.scale || 1)
    var ew = e.width / (e.scale || 1)
    var eh = e.height / (e.scale || 1)
    var epsilon = 1
    if (e.x + ew <= i.x + epsilon) return "left"
    if (e.x >= i.x + iw - epsilon) return "right"
    if (e.y + eh <= i.y + epsilon) return "top"
    return ""
  }

  function setArrangement(direction) {
    actionProc.command = [root.scriptPath("arrange.sh"), direction]
    if (!actionProc.running) actionProc.running = true
  }

  // Resolves alongside this plugin's own Panel.qml, so it keeps working
  // however the clone gets renamed or relocated.
  ...
  readonly property var visibleSections: {
    ...
    if (arrangementAvailable) list.push("arrangement")
    return list
  }
  ...
  function sectionCount(section) {
    ...
    if (section === "arrangement") return arrangementOptions.length
    return 0
  }

  function sectionIsSingleRow(section) {
    // brightness and text size are lone sliders; scale/arrangement presets sit horizontally.
    return section === "brightness" || section === "textsize" || section === "scale" || section === "arrangement"
  }
  ...
  // h/l: in scale/arrangement sections, walks the preset row; everywhere
  // else, no-op because adjustBrightness handles horizontal motion on the
  // brightness slider.
  function moveCursorH(delta) {
    var count = 0
    if (focusSection === "scale") count = scaleValues.length
    else if (focusSection === "arrangement") count = arrangementOptions.length
    else return
  ...
  function activateCursor() {
    ...
    if (focusSection === "arrangement" && selectedIndex >= 0 && selectedIndex < arrangementOptions.length) {
      setArrangement(arrangementOptions[selectedIndex].value)
      return
    }
  ...
  onArrangementAvailableChanged: clampCursor()
  ...
  Process {
    id: stateProc
    command: [root.scriptPath("state.sh")]
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---------- Arrangement ----------
          PanelSeparator {
            visible: root.arrangementAvailable
            foreground: root.bar.foreground
          }
          ...
          Column {
            ...
            Grid {
              id: arrangementRow
              width: parent.width
              columns: root.arrangementOptions.length
              spacing: Style.spacing.xs

              readonly property real cellWidth: root.arrangementOptions.length > 0
                ? (width - spacing * (columns - 1)) / columns
                : 0

              Repeater {
                model: root.arrangementOptions
                ...
                ArrangementPill {
                  ...
                  optionIndex: index
                  width: arrangementRow.cellWidth
                }
              }
            }
          }
  ...
  component ArrangementPill: Button {
    id: arrangementPill
    ...
    active: root.currentArrangement === optionValue
    hasCursor: root.cursorActive && root.focusSection === "arrangement" && root.selectedIndex === optionIndex

    onClicked: root.setArrangement(optionValue)
    onHovered: function(isHovered) {
      if (!isHovered || root.reflowingText) return
      root.cursorActive = true
      root.focusSection = "arrangement"
      root.selectedIndex = arrangementPill.optionIndex
    }
  }
